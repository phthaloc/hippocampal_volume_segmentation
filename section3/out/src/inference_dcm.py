...
if __name__ == '__main__':
    ...
    # routed studies (here: "/data/TestVolumes/StudyX" with X in {1,2,3})
    if len(sys.argv) != 2:
        print('You should supply one command line argument pointing ' +
              'to the routing folder (test volumes directory). In out case run something like: ' +
              "python '/data/TestVolumes/Study1'. Do not forget to edit file " +
              ' deploy_scripts/start_listener.sh and enter same directory in -od argument. Exiting.')
        sys.exit()
    ...
    inference_agent = UNetInferenceAgent(
        device='cpu',
        parameter_file_path=r'../parameters/weights.weights',
        patch_size=64,
        model=None)
    ...
    report_img = create_report(inference=pred_volumes, header=header, orig_vol=volume, pred_vol=pred_label)
    save_report_as_dcm(headeer=header, report=report_img, path=report_save_path)
